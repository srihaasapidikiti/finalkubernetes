...
connection = pika.BlockingConnection(
    pika.ConnectionParameters('rabbitmq', 5672, heartbeat=0))
...
def test1(locality, cuisines, cost):
    ...
    # build a response dict to send back to client
        ...
        channel.basic_publish(
           exchange='topic_logs', routing_key=hname+'.rest.logs', body='checked successfully for restaurants')        
    except:
        response = {'Restaurant':None}
        channel.basic_publish(
            exchange='topic_logs', routing_key=hname+'.rest.debug', body='failed getting reviews')
    # encode response using jsonpickle
    response_pickled = jsonpickle.encode(response)
